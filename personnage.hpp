...
public:
    Partiel() : Evenement("Partiel","Un examen très très important pour évaluer vos compétences.") {
        vector<pair<string, string>> questions_faciles = create_questions("facile");
        vector<pair<string, string>> questions_moyennes = create_questions("moyen");
        vector<pair<string, string>> questions_difficiles = create_questions("difficile");
    }

    void executer(Stats& stats) override {
        std::cout << "Vous passez un partiel...\n";
        ...
        std::cout << "Vous avez obtenu " << resultat << "/20." << std::endl << "Nouvelle note moyenne : " << stats.note << "/20.\n";
        if (stats.energie <= 0) {
            stats.energie = 0;
            std::cout << "Attention, votre énergie est à 0.\n";
        }
...
class Repos : public Evenement {
    ...
    void executer(Stats& stats) override {
        std::cout << getDescription() << std::endl;
        stats.energie += 5;
        std::cout<< "Vous gagnez en enérgie.\n\n";
...
class Vacances : public Evenement {
    ...
    void executer(Stats& stats) override {
        std::cout << getDescription() << std::endl;
        stats.energie += 20;
        std::cout << "Vous avez gagné plein d'enérgie.\n\n";
