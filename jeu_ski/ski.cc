...
const int GATE_WIDTH = 150;  // Augmenter la taille des portes
const int GATE_SPACING = 300;  // Réduire davantage la fréquence des portes
const float PLAYER_SPEED = 0.2;
const float GATE_SPEED = 0.1;  // Réduire encore plus la vitesse des portes
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (score % 5 == 0 && score > 0) {
            difficulty += 0.0001f;  // Augmente la difficulté toutes les 5 points de score
        }
        ...
        if (!gameOver) {
            ...
            for (auto it = gates.begin(); it != gates.end();) {
                it->yPosition += difficulty*GATE_SPEED;
                it->leftPost.move(0, difficulty*GATE_SPEED);
                it->rightPost.move(0, difficulty*GATE_SPEED);
            ...
            for (auto& snowDune : buffer_snow) {
                snowDune.neige.move(0, difficulty*.1f);  // Déplacer les dunes de neige vers le bas
            }
